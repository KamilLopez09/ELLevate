...
import { isClickPaintPrompt } from "@/lib/prompt-utils";
import { useCopy } from "@/lib/i18n/useCopy";
import type { GameModeProps } from "@/types/game-modes";

export function RapidFire({ prompts, onComplete }: GameModeProps) {
  const copy = useCopy();
  const clickPrompts = prompts.filter(isClickPaintPrompt);
  const promptId = clickPrompts[0]?.id ?? "";
  ...
  return (
    <div className="flex w-full flex-col items-center">
      ...
      <SentenceBuilder
        prompts={clickPrompts}
        gameModeId="rapid_fire"
